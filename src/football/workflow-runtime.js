...
// → 單張能量表單 → 雙牌源賽後回顧分流 → 雙牌源同場比較 → 現行能量統計
// → 運彩單注與比分結算。雲端與事件由 application-runtime.js 在本層完成後建立。
//
// 主要流程複雜度：
// - 模組初始化：時間／空間 O(W)，W 為固定 8 個相容流程模組。
// - 執行層契約檢查：時間／空間 O(1)。
//
// 更快替代方案比較：
// - 事件與雲端直接讀取任意時點的 window 核心，會依賴隱性載入順序。
// - 本層固定淘汰賽、雙牌源回顧分流、雙牌源比較、現行統計與運彩包裝完成的核心／Render，後續模組只接收這份快照。
...
if (
  ...
  throw new Error("世足淘汰賽／雙牌源／運彩流程尚未正確包裝核心與 Render。");
}
...
export const footballWorkflowRuntime = Object.freeze({
  ...
  betting: footballBettingRuntime,
  core: workflowCore,
