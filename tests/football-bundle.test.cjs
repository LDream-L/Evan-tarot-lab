...
/**
 * 驗證世足 ES Module entry、正式 bundle、具名模型與 sourcemap。
 * 時間／空間複雜度 O(B)，B 為入口、相容模組、bundle 與 map 的總大小。
 *
 * 替代方案比較：只檢查 bundle 存在無法證明依賴是否納入；
 * 本測試同時核對 25 個相容 side-effect imports、5 個具名 imports、
 * 舊混合模組移除、不可變資料契約與 map sources。
 */
function run() {
  ...
  assert.equal(sideEffectImports.length, 25, "世足入口應保留 25 個相容 side-effect imports");
  assert.equal(namedImports.length, 5, "世足資料、核心、評分、能量模型與轉接層必須使用 5 個具名 imports");
