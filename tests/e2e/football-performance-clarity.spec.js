...
test("滾動績效卡片顯示本期、比較期、差異與明確樣本限制", async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    const byOriginalLabel = (label) => cards.find((card) => (
      card.querySelector(":scope > small")?.textContent.trim() === label
    ));
    const setBase = (label, value, detail) => {
      const card = byOriginalLabel(label);
    ...
    const structure = setBase("攻防推導賽果", "40%", "4／10");
    ...
    const agreement = setBase("雙模型同判斷一致率", "90%", "9／10");
