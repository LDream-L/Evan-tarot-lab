...
- Google Sheets：使用者資料、文章、服務、方案、圖片索引與研究紀錄的雲端資料來源。
- Google Drive：文章後台上傳圖片的正式檔案儲存位置。
- Apps Script：Google Sheets 與 Google Drive 的正式後端。
...
│  ├─ ArticleMedia.gs          # Google Drive 圖片與 ArticleMedia 索引
...
- `npm run test:build`：檢查導覽、品牌、登入依賴、服務方案、文章圖片、模組邊界、資料界線與正式輸出。
...
- 文章圖片功能：除更新 `Code.gs` 外，必須新增或更新 `ArticleMedia.gs`，執行 `setupArticleMediaLibrary` 後再重新部署原 Web App。
...
總體 `health` 必須同時確認 `authReady`、文章、文章圖片與服務資料，而不是只確認 OAuth Client ID 有值。

## 安全與資料界線

- Client Secret、管理員密碼、Spreadsheet ID、Drive 私人檔案 ID 與 service account 金鑰不得提交至 GitHub。
- 管理員按鈕是否顯示不是權限依據；Apps Script 必須再次驗證 Google ID Token 與 `ADMIN_EMAILS`。
- `ARTICLE_ADMIN_EMAILS` 已淘汰，文章、圖片與服務統一使用 `ADMIN_EMAILS`。
- `cloud/`、`tests/`、`src/` 與建置設定不會複製到公開 `dist/`。
- 預約、文章、文章圖片、服務與私人紀錄使用不同資料流程；公開頁只取得必要的公開欄位。
